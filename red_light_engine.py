# Auto-generated headless red-light processor for unified pipeline.
...
class RedLightPipelineEngine:
    ...
    def __init__(self, project_dir: str | None = None):
        self.root = Path(project_dir or Path(__file__).resolve().parent)
        os.chdir(self.root)
        with open(self.root / "roi_config.json", "r", encoding="utf-8") as f:
            cfg = json.load(f)
        self.violation_rois = list(cfg.get("violation_rois") or cfg.get("rois") or [])
        sr = cfg.get("signal_roi")
        self.signal_roi = list(sr) if sr and len(sr) == 4 else None
        if not self.violation_rois or not self.signal_roi:
            raise ValueError("roi_config.json must define violation_rois and signal_roi")
        with open(self.root / "coco.txt", "r") as f:
        ...
        self.output_dir = os.path.join(str(self.root), "saved_images", today)
    ...
    def process_frame(self, frame_bgr, frame_idx: int, time_sec: float):
        from test1 import process_frame as tf_process_frame
        ...
        processed_frame, detected_label = tf_process_frame(frame, signal_roi=current_signal_roi)
        ...
        for bbox in bbox_idx:
            ...
            def save_and_event():
                ok = pm._save_violation(
                    frame, obj_class, oid, (x3, y3, x4, y4),
                    self.output_dir, self.violated_ids, self.plate_reader,
                )
                if ok:
                    events.append(
                        {
            ...
            if in_any_roi and not is_target:
                ...
                if inside_zone:
                    ...
                    crossed_now = bool(
                        ...
                        and pm._crossed_stop_line(prev_center, (cx, cy), matched_roi, pm.TRACK_DIRECTION)
                    )
                    if _red_ok() and crossed_now:
                        cvzone.putTextRect(frame, f"{obj_class} {oid} VIOLATION", (x3, y3), 1, 1)
                        ...
                        if move_ok:
                            ...
                            cvzone.putTextRect(frame, f"{obj_class} {oid} APPROACH", (x3, y3), 1, 1)
                            cv2.rectangle(frame, (x3, y3), (x4, y4), (0, 165, 255), 2)
                        else:
                            cvzone.putTextRect(frame, f"{obj_class} {oid} IGNORE", (x3, y3), 1, 1)
                    ...
                    if prev_center is not None and _red_ok():
                        ...
                        crossed_line = bool(
                            ref_roi
                            and pm._crossed_stop_line(prev_center, (cx, cy), ref_roi, pm.TRACK_DIRECTION)
                        )
                        if (
                            ref_roi
                            and pm._exited_forward(cx, cy, ref_roi, pm.TRACK_DIRECTION, prev_center)
        ...
        for idx, roi in enumerate(self.violation_rois):
            ...
            cv2.putText(
                frame, f"V{idx + 1}", (rx1, max(20, ry1 - 5)),
